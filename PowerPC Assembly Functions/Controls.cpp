...
//does some convoluted stuff to make name list not reopen when picking an existing name with replacing
void NameIsFound()
{
	//r29 + 0x1FC == menu ptr
	...
	SetRegister(Reg1, 1);
	STB(Reg1, 29, MENU_STATE_INFO_OFFSET + 10 + 0x1FC);

	ASMEnd(0x38810020); //addi r4, sp, 32



	ASMStart(0x8069b890);

	LBZ(6, 29, MENU_STATE_INFO_OFFSET + 10 + 0x1FC);
	STB(6, 29, MENU_STATE_INFO_OFFSET + 0x1FC);

	ASMEnd(0x2c130000); //cmpwi r19, 0
}
...
void ReplaceName()
{
	...
	STB(Reg1, 29, MENU_STATE_INFO_OFFSET + 10 + 0x1FC); //clear open flag
